...
export function AiAssistant() {
  ...
  const fetchDailySummary = async () => {
    ...
    try {
      ...
      if (parsedSummary) {
        ...
        setSummaryLoadingError(t("assistant.summaryErrors.parseFailed"));
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    if (activeSessionId) {
      sessionStorage.setItem(ACTIVE_CHAT_SESSION_STORAGE_KEY, activeSessionId);
    } else {
  ...
  const sendMessageMutation = useMutation({
    ...
    }) => chatService.sendMessage(sessionId, role, content, metadata),
    onSuccess: () => {
  ...
  const handleSearch = async (q: string = query) => {
    ...
    try {
      ...
      if (webhookUrl) {
        ...
        responseText = webhookResult.answer;
        responseMetadata = webhookResult.metadata || {};
      }
      ...
      let assistantSaveError: unknown = null;
      const saveAssistantMessage = sendMessageMutation
        .mutateAsync({
          sessionId: currentSessionId,
          role: "assistant",
      ...
      streamIntervalRef.current = setInterval(() => {
        ...
        updateMessages(prev =>
          prev.map(msg =>
            msg.id === aiMsgId ? { ...msg, content: currentContent } : msg
          )
        );
        ...
        if (charIndex >= responseText.length) {
          ...
          // 8. Assistant is already visible locally; persist it with webhook metadata before refetch unlocks.
          saveAssistantMessage
            .then(async () => {
              if (assistantSaveError) {
                console.error("Failed to save AI message:", assistantSaveError);
      ...
      toast.error(t("assistant.toasts.aiError", { message: errorMessage }));
    }
  };
                ...
                >
                  ...
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
                    <button
                      ...
                      title={t("assistant.sidebar.renameChat")}
                    >
                    ...
                    <button
                      ...
                      title={t("assistant.sidebar.deleteChat")}
                    >
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-500 mt-2">
                    {t("assistant.summaryLoading.description")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <AiDetailSummary
              ...
              data={
                summaryData ?? {
                  summaryCards: [],
                  insight: {
                    title: t("assistant.summaryFallback.title"),
                    message: t("assistant.summaryFallback.message"),
                    recommendation: t(
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!isSidebarOpen && (
                    <button
                      ...
                      title={t("assistant.sidebar.viewChatHistory")}
                    >
                          ...
                          <div className="relative flex flex-col justify-between h-full z-10 gap-4">
                            ...
                            <div>
                              <span className="text-lg font-bold text-white block mb-1">
                                {t("assistant.emptyState.detailSummaryTitle")}
                              </span>
              ...
              <div className="relative group max-w-3xl mx-auto">
                ...
                <div className="relative bg-white dark:bg-zinc-700 rounded-[1.5rem] shadow-sm hover:shadow-md transition-all duration-300 border border-slate-200 dark:border-zinc-600 p-1.5 flex items-center gap-2 pr-2">
                  ...
                  <textarea
                    ...
                    placeholder={
                      isListening
                        ? t("assistant.input.listeningPlaceholder")
                        : t("assistant.input.askPlaceholder")
                    }
                  ...
                  {query.trim() ? (
                    ...
                    <button
                      ...
                      title={
                        isListening
                          ? t("assistant.input.stopListening")
                          : t("assistant.input.startVoiceInput")
                      }
                    >
                      {isListening ? (
