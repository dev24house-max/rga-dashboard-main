                ...
                <div className="container px-4 mx-auto max-w-6xl space-y-24">
                    ...
                    <div className="space-y-16">
                        <div className="text-center space-y-4 max-w-3xl mx-auto">
                            ...
                            <div className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide">Powering Growth</div>
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {[
                                ...
                                <div key={item} className="group flex items-start gap-4 p-6 rounded-2xl bg-white/50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-700 shadow-sm hover:shadow-md hover:bg-white dark:hover:bg-zinc-800 hover:border-indigo-100 dark:hover:border-indigo-500/30 transition-all duration-300">
                                    ...
                                    <div className="text-base font-medium text-slate-700 dark:text-zinc-300 pt-1 group-hover:text-slate-900 dark:group-hover:text-zinc-100 transition-colors">{item}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div id="pricing" className="space-y-12 relative">
                        ...
                        <div className="text-center space-y-4">
                            <h3 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-zinc-100">Simple, Transparent Pricing</h3>
                            <p className="text-lg text-slate-600 dark:text-zinc-400">No hidden fees. Choose the plan that fits your business needs.</p>
                        </div>
    ...
    return (
        <Card className={cn(
            ...
            <CardHeader className="space-y-2 pb-6 pt-8">
                <CardTitle className="text-2xl font-bold text-slate-900 dark:text-zinc-100">{plan.name}</CardTitle>
                <div className="text-sm text-slate-500 dark:text-zinc-500 h-10 leading-snug">{plan.description}</div>
            </CardHeader>

            <CardContent className="flex-1 space-y-8">
                <div className="flex items-baseline gap-1">
                    <span className="text-5xl font-extrabold text-slate-900 dark:text-zinc-100 tracking-tight">{formatCurrency(plan.price)}</span>
                    <span className="text-base font-medium text-slate-500 dark:text-zinc-500">/mo</span>
                </div>

                <ul className="space-y-4">
                    {plan.features.map((feature) => (
                        <li key={feature} className="flex items-start gap-3 text-sm text-slate-700 dark:text-zinc-300">
                            <div className={cn(
                                "mt-0.5 p-0.5 rounded-full shrink-0",
                                plan.highlight ? "bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400" : "bg-slate-100 dark:bg-zinc-700 text-slate-600 dark:text-zinc-400"
                            )}>
