...
const PROTECTED_MATCHER_PATHS = new Set([
  "/login",
  "/register",
  "/forgot-password",
  "/reset-password",
...
export function useTutorialFlow() {
  ...
  const config: TutorialConfig | undefined =
    tutorialRegistry[currentTutorialKey];
  ...
  const steps = useMemo(
    () =>
      rawSteps.map(step => {
        ...
        const descriptionKey = `steps.${step.id}.description`;
        const title = t(titleKey);
        const description = t(descriptionKey);

        return {
          ...step,
          title: title === titleKey ? step.title : title,
          description:
            description === descriptionKey ? step.description : description,
        };
      }),
    [rawSteps, t]
  );
  ...
  useEffect(() => {
    const onStart = (event: Event) => {
      ...
      const requestedPath = customEvent?.detail?.pagePath
        ? normalizePath(customEvent.detail.pagePath)
        : currentPath;
      ...
      if (
        ...
        !PROTECTED_MATCHER_PATHS.has(requestedPath)
      ) {
    ...
    return () =>
      window.removeEventListener("start-tutorial", onStart as EventListener);
