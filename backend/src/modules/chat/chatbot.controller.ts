...
export class ChatbotController {
  ...
  async forwardToWebhook(@Body() body: ChatRequest): Promise<ChatResponse> {
    ...
    const n8nBaseUrl =
      process.env.N8N_BASE_URL || "https://yourrga3.app.n8n.cloud";
    ...
    try {
      const timeoutMs = parseInt(
        process.env.N8N_REQUEST_TIMEOUT_MS || "120000",
        10,
      );
      ...
      if (!resp.ok) {
        console.error(
          "[Chatbot] N8N error response:",
          text,
          "Status:",
          resp.status,
        );
        console.error(
          "[Chatbot] Check N8N webhook URL and ensure the workflow version is published",
        );
      ...
      if (!text || text.trim() === "") {
        ...
        console.warn(
          "[Chatbot] Response headers:",
          Object.fromEntries(resp.headers.entries()),
        );
        console.warn("[Chatbot] Target URL:", target);
        console.warn(
          "[Chatbot] Check N8N webhook configuration and ensure workflow is published",
        );
        return {
          success: false,
          error:
            "N8N returned empty response - please check webhook configuration",
          reply: "",
          message: "",
          timestamp: new Date().toISOString(),
        };
      ...
      if (contentType.includes("application/json")) {
        try {
          ...
          console.warn(
            "[Chatbot] Failed to parse N8N response as JSON:",
            parseErr,
          );
      ...
      console.log("[Chatbot] N8N Status:", resp.status);
      console.log(
        "[Chatbot] N8N Response Headers:",
        Object.fromEntries(resp.headers.entries()),
      );
      console.log("[Chatbot] N8N Response Raw Text:", text);
      console.log(
        "[Chatbot] N8N Response Data:",
        JSON.stringify(n8nData, null, 2),
      );
      ...
      return {
        ...
        message: reply,
        output: n8nData,
        metadata: n8nData?.metadata || n8nData?.data?.metadata || {},
        timestamp: new Date().toISOString(),
      };
    } catch (err: any) {
      ...
      return {
        ...
        reply:
          "Unable to connect to AI service. Please check your connection and N8N webhook URL.",
        message:
          "Unable to connect to AI service. Please check your connection and N8N webhook URL.",
        timestamp: new Date().toISOString(),
      };
