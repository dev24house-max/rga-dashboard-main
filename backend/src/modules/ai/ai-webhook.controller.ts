...
export class AiWebhookController {
  ...
  private async proxyToN8n(webhookEnv: string, body: any) {
    ...
    console.log(
      `[AiWebhookController] Using env: ${webhookEnv} = ${webhookUrl}`,
    );
    ...
    try {
      ...
      if (typeof n8nData === "string" && /<html|<!doctype/i.test(n8nData)) {
        throw new HttpException(
          "Invalid HTML response from webhook. Check your n8n webhook URL.",
      ...
      const hasReply =
        reply != null && (typeof reply !== "string" || reply.trim().length > 0);
      const isHtmlReply =
        typeof reply === "string" &&
        /<\/?(?:html|head|body|div|span|script|title|meta|!doctype)/i.test(
          reply.trim(),
        );

      if (!hasReply || isHtmlReply) {
        throw new HttpException(
          "No valid AI response from webhook (received HTML or empty content)",
          HttpStatus.BAD_GATEWAY,
        );
      }
